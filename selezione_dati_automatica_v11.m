...
%calcolo fattore di Oswald al variare dell'angolo di freccia
...
[~,c]=size(Tab_variazioni);                             % 1) estraggo la dimensione dell'array Tab_variazioni per il ciclo FOR
...
% creo una tabella contente la variazione % rispetto alla geometria 
% con angolo di freccia nullo
...
if length(somma_punteggi)~=length(unique(somma_punteggi))
    warning('ci sono piu'' valori uguali nel vettore somma_punteggi, verificare se il valore presente con molteplicita>1 corrisponde al massimo')
end
...
% elimino variabili inutili
 clear col col1 col2 i row row1 row2 row3 row4 row5 c
